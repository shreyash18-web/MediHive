...
export const SupabaseConnectionModal: React.FC<SupabaseConnectionModalProps> = ({
  ...
  const [config, setConfig] = useState<SupabaseConfigInfo>(() => getSupabaseConfig());
  ...
  const handleSave = async () => {
    ...
    setConfig(updated);
    showToast('Supabase credentials saved successfully! Reloading clinic data...', 'success');
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-2 sm:p-4">
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-2xl w-full max-h-[96dvh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        {/* Header */}
        <div className="px-5 py-4 bg-gradient-to-r from-[#194358] via-[#205570] to-[#2c7295] text-white flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-sky-100/90">
                Connect your MediHive clinic to PostgreSQL cloud storage & multi-device sync
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5 text-xs text-slate-700 touch-scroll">
          ...
          {testResult && testResult.tested ? (
            ...
            >
              ...
              <div className="space-y-1 text-xs">
                ...
                {testResult.details && (
                  <p className={`text-[11px] ${testResult.success ? 'text-emerald-700' : 'text-amber-800'}`}>
                    {testResult.details}
                  </p>
                )}
              </div>
            ...
            <div className="p-3.5 rounded-xl border bg-sky-50 border-sky-200 text-sky-900 flex items-center gap-3">
              ...
              <span className="font-medium text-xs">Testing connection to Supabase...</span>
            </div>
          ) : null}
          ...
          {config.isPlaceholder && (
            <div className="bg-rose-50 border border-rose-200 rounded-xl p-3.5 text-rose-900 space-y-1.5">
              ...
              <p className="text-[11px] text-rose-800 leading-relaxed">
                ...
                <code className="bg-rose-100 px-1 py-0.5 rounded font-mono text-rose-950 font-bold">https://your-project-id.supabase.co</code>.
                To sync your patients and prescriptions to the cloud, paste your real <strong>Project URL</strong> and <strong>Anon Key</strong> below.
              </p>
            </div>
          )}
          ...
          <div className="space-y-3.5 bg-slate-50 p-4 rounded-xl border border-slate-200">
            ...
            <div>
              ...
              />
              <p className="text-[11px] text-slate-400 mt-1">
                Found in: Supabase Dashboard &gt; Project Settings &gt; API &gt; Project API keys &gt; <strong>anon public</strong>
              </p>
            </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-2.5">
            ...
            <ol className="list-decimal list-inside space-y-1.5 text-[11px] text-slate-600 leading-relaxed pl-1">
              ...
              <li>
                In the bottom-left sidebar, click <strong>⚙️ Project Settings &gt; API</strong> (or <strong>Data API</strong>).
              </li>
              <li>
                Under <strong>Project URL</strong>, copy the URL (e.g. <code className="font-mono bg-slate-100 px-1 py-0.5 rounded">https://xyz.supabase.co</code>).
              </li>
              <li>
                Under <strong>Project API keys</strong>, copy the key labeled <strong>anon public</strong> (starts with <code className="font-mono bg-slate-100 px-1 py-0.5 rounded">eyJ...</code>).
              </li>
              <li>
                Paste them into the fields above and click <strong>Save & Connect</strong> (or update your <code className="font-mono bg-slate-100 px-1 py-0.5 rounded">.env</code> file).
              </li>
              <li>
                Ensure you executed the SQL script from <code className="font-mono bg-slate-100 px-1 py-0.5 rounded">supabase_schema.sql</code> in the Supabase <strong>SQL Editor</strong> to create all tables!
              </li>
            </ol>
          </div>
        </div>
        ...
        <div className="px-5 py-3 bg-slate-50 border-t border-slate-200 flex items-center justify-between shrink-0">
          <span className="text-[11px] text-slate-400">
            Current source: <strong className="uppercase font-mono">{config.source}</strong>
          </span>
          <button
            type="button"
            onClick={onClose}
